...
{ 
  ...
  // #1
      int rem_bt[n];
      for (int i = 0; i < n; i++) {
      rem_bt[i] = plist[i].bt; // assign burst time to remaining burst time
      plist[i].wt = 0; // #2
      }
...
{ 
  ...
  for (int i = 0; i < n; i ++){
    ...
    int minimum = 0;
		int curr_min = plist[0].wt;
		for(int j = 1; j < n; j++) {
			if (plist[i].wt < curr_min) {
				curr_min = plist[j].wt;
				minimum = j;
			}
		}
      ...
      plist[minimum].wt--;
      if (plist[minimum].wt == 0) {
        completedProcesses++;
        break;
      }
      completionTime[minimum] = plist[minimum].wt + 1;
      plist[minimum].wt = completionTime[minimum] - plist[minimum].art - plist[minimum].bt;
  }
} 
...
{ 
     ...
     */ 
    ProcessType *comp1 = (ProcessType*)this;
    ProcessType *comp2 = (ProcessType*)that;
    return (comp1->pri < comp2->pri ? 1 : 0);
